...
/-
LegalKernel.Test.Properties.Lex — Workstream-LX property-based
tests.
...
properties land for the LX.21 acceptance gate:
  ...
  2. `lex_codegen_determinism_property` — running every renderer
     in `Tools/LexCodegen.lean` twice on the same input produces
  ...
  3. `lex_diff_reformatting_invariance_property` — applying the
     fence-rewriting helpers in `Tools/LexCodegen.lean` to the
