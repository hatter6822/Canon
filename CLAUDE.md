...
**Test count.**  ~2042 tests across ~101 suites at the time of
the SC.1 milestone (Workstream SC.1, +56 from EI's 1986 — 53
new tests in the `faultproof-smt` suite plus 3 scattered API-
stability checks).  At the EI milestone the count was ~1986
across ~100 suites, up from 1907 at the AR milestone (+79).
The exact number drifts with every PR; `lake test` is the
canonical query.  Unlike the build tag, the test count is not
pinned — only its monotonic growth is enforced by individual
regression tests landing alongside new theorems.
