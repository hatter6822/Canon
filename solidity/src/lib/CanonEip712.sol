...
library CanonEip712 {
    ...
    string internal constant CANON_ACTION_TYPE_STRING =
        "CanonAction(bytes32 actionHash,uint64 signer,uint64 nonce,bytes deploymentId)";

    string internal constant CANON_MIGRATION_TYPE_STRING =
    ...
    /// @notice Type hash for the `CanonAction` struct.
    function canonActionTypeHash() internal pure returns (bytes32) {
        return keccak256(bytes(CANON_ACTION_TYPE_STRING));
    }

    /// @notice Type hash for the `CanonMigration` struct.
    function canonMigrationTypeHash() internal pure returns (bytes32) {
        return keccak256(bytes(CANON_MIGRATION_TYPE_STRING));
    }
