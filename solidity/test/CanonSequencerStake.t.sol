...
contract CanonSequencerStakeTest is Test {
    ...
    function test_deposit_reverts_on_non_sequencer() public {
        vm.expectRevert(CanonSequencerStake.NotSequencer.selector);
    ...
    function test_withdraw_reverts_on_zero_amount() public {
        vm.prank(sequencer);
        stake.deposit{value: 10 ether}();
        vm.expectRevert(CanonSequencerStake.InsufficientStake.selector);
        vm.prank(sequencer);
        stake.withdraw(0);
    }
    ...
    function test_withdraw_blocked_during_dispute_window() public {
        ...
        vm.expectRevert(CanonSequencerStake.WithdrawDuringOpenDispute.selector);
        vm.prank(sequencer);
        stake.withdraw(1 ether);
    }
    ...
    function test_slash_reverts_on_non_disputeVerifier() public {
        vm.prank(sequencer);
        stake.deposit{value: 10 ether}();
        vm.expectRevert(CanonSequencerStake.NotDisputeVerifier.selector);
        vm.prank(notSequencer);
    ...
    function test_slash_idempotent_per_disputeId() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(CanonSequencerStake.AlreadySlashed.selector, uint64(1))
        );
        vm.prank(address(verifier));
        stake.slash(1, challenger);
    }
    ...
    function test_slash_reverts_on_zero_challenger() public {
        vm.prank(sequencer);
        stake.deposit{value: 10 ether}();
        vm.expectRevert(CanonSequencerStake.ZeroAddress.selector);
        vm.prank(address(verifier));
