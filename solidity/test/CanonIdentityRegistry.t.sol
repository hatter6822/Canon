...
contract CanonIdentityRegistryTest is Test {
    CanonIdentityRegistry private reg;
    ...
    function setUp() public {
        reg = new CanonIdentityRegistry(CANON_VERSION);
    ...
    function test_constructor_sets_deploymentId() public view {
        bytes32 expected =
            keccak256(abi.encode(block.chainid, address(reg), CANON_VERSION));
    ...
    function test_registerECDSA_happy_path() public {
        ...
        ICanonIdentityRegistry.IdentityRecord memory rec = reg.lookup(alice);
        assertEq(uint256(rec.kind), uint256(ICanonIdentityRegistry.SignerKind.ECDSA_EOA));
    ...
    function test_registerECDSA_revert_on_pubkey_address_mismatch() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CanonIdentityRegistry.PubkeyAddressMismatch.selector,
                eve,
                alice
            )
        );
    ...
    function test_registerECDSA_revert_on_wrong_pubkey_length() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(CanonIdentityRegistry.WrongPubkeyLength.selector, 4)
        );
    ...
    function test_registerECDSA_revert_on_re_registration() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(CanonIdentityRegistry.AlreadyRegistered.selector, alice)
        );
        reg.registerECDSA(alicePubkey);
        vm.stopPrank();
    }
    ...
    function test_registerEIP1271_happy_path_accept_all() public {
        ...
        ICanonIdentityRegistry.IdentityRecord memory rec = reg.lookup(user);
        assertEq(
            uint256(rec.kind),
            uint256(ICanonIdentityRegistry.SignerKind.EIP1271_CONTRACT)
        );
    ...
    function test_registerEIP1271_revert_on_bad_magic() public {
        ...
        vm.prank(user);
        vm.expectRevert(CanonIdentityRegistry.NotEip1271Conforming.selector);
        reg.registerEIP1271(address(signer_));
    }
    ...
    function test_registerEIP1271_revert_on_re_registration() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(CanonIdentityRegistry.AlreadyRegistered.selector, user)
        );
        reg.registerEIP1271(address(signer_));
        vm.stopPrank();
    }
    ...
    function test_revoke_clears_record() public {
        ...
        assertEq(
            uint256(rec.kind),
            uint256(ICanonIdentityRegistry.SignerKind.UNREGISTERED)
        );
        vm.stopPrank();
    }
    ...
    function test_revoke_reverts_when_not_registered() public {
        vm.expectRevert(
            abi.encodeWithSelector(CanonIdentityRegistry.NotRegistered.selector, alice)
        );
        vm.prank(alice);
        reg.revoke();
    }
